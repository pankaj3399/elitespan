...
const PayPalForm = ({ onClose, onContinue, userId, token: propToken }) => {
  ...
  useEffect(() => {
    const fetchPaymentIntent = async () => {
      ...
      try {
        const response = await createPaymentIntent(finalToken, {
          amount: 9.99,
          userId,
          doctorId: null,
        });
        ...
        setError(
          err.message ||
            "Payment intent creation failed. Please ensure you’re logged in."
        );
      }
    };
    fetchPaymentIntent();
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("paymentSuccess", handleSuccessCallback);
    ...
    <div className="fixed inset-0 backdrop-blur-md bg-opacity-0 flex items-center justify-center z-50">
      <div className="bg-white p-8 rounded-3xl shadow-lg relative max-w-md w-full mx-4">
        ...
        <p className="text-center text-gray-600 mb-8">
          You&apos;ll be redirected to PayPal to complete your payment. Lorem
